...
class EditTaskFragment : Fragment() {
    ...
    private var initialCompleted: Boolean = false
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        arguments?.let {
            ...
            val dateStr      = it.getString("taskDate", "")
            val timeStr      = it.getString("taskTime", "")
            initialCompleted = it.getBoolean("taskCompleted", false)

            etDate.setText(dateStr)
            etTime.setText(timeStr)
            when (it.getInt("taskImportance", 1)) {
                1 -> rgImportance.check(R.id.radio_1)
                2 -> rgImportance.check(R.id.radio_2)
                3 -> rgImportance.check(R.id.radio_3)
            }
        }

        etDate.setOnClickListener {
            val parts = etDate.text.toString().split("-").map { it.toInt() }
